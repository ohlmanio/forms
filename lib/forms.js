...
function initalize(variableName) {
  var initalizer = (
    ...
    if (_.isUndefined(fieldName)) {
      ...
      if (hasNewValue) {
        doc = Tracker.nonreactive(function () {
          return self[variableName].get() || initalize(variableName);
        });
...
Forms.onCreated(function (options) {
  ...
  tmpl.autorun(function () {
    ...
    tmpl.form.original(data && data.doc ? EJSON.clone(data.doc) : options.doc || {});
    tmpl.form.schema(data && data.schema ? EJSON.clone(data.schema) : options.schema || {});
...
Forms.methods({
  ...
  , validate: makeEventTrigger(function (propertyName) {
    ...
    var errors = _.map(schema, function (rule, propertyName) {
      ...
      var error = rule.call(form, value, propertyName);
